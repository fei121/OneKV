...
> vLLM/SGLang 用**修正后的上下文**重测（3B/7B 皆然：vLLM `--max-model-len 32768`、SGLang `--max-total-tokens 49152`）。
...
| metric | shared-KV engine | llama.cpp | vLLM | SGLang |
|---|---|---|---|---|
| throughput (tok/s) | 84.6→103.3 | 81.9→92.4 | 88.8→123.6 | 90.3→125.2 |
| TPOT p95 (ms) | **20.8→22.3** | 65.0→125.8 | 37.9→46.0 | 31.3→44.6 |
| cold TTFT (ms) | **524.2→625.0** | 800.6→1179.0 | 591.1→1214.5 | 590.5→958.6 |

**Plan-and-Execute**

| metric | shared-KV engine | llama.cpp | vLLM | SGLang |
|---|---|---|---|---|
| throughput (tok/s) | 93.2→115.8 | 92.1→108.1 | 94.8→131.3 | 97.2→134.5 |
| TPOT p95 (ms) | **20.4→22.1** | 71.5→111.1 | 21.6→110.0 | 25.6→48.5 |
| cold TTFT (ms) | **540.9→646.0** | 803.2→1252.2 | 648.5→1036.0 | 782.8→945.2 |
